...
const T=process.env.DISCORD_BOT_TOKEN,C=process.env.DISCORD_CHANNEL_ID,P=Number(process.env.PORT)||3000,I=10000;
...
async function poll(ch){try{const fs=await fetchAll();fs.sort((a,b)=>new Date(a.properties.time)-new Date(b.properties.time));for(const f of fs){const k=key(f);if(seen.has(k))continue;seen.add(k);const t=new Date(f.properties.time);if(!last||t>last)last=t;await ch.send({embeds:[embed(f)]});console.log("Posted",f.src,k);}if(seen.size>2000){const a=[...seen];seen.clear();a.slice(-1000).forEach(x=>seen.add(x));}}catch(e){console.error(e);}}
const c=new Client({intents:[GatewayIntentBits.Guilds]});
c.once("clientReady",async()=>{console.log("Logged in as "+c.user.tag);const ch=await c.channels.fetch(C);const init=await fetchAll();for(const f of init){seen.add(key(f));const t=new Date(f.properties.time);if(!last||t>last)last=t;}await ch.send({embeds:[new EmbedBuilder().setColor(0x2ecc71).setTitle("🌍 Monitoraggio terremoti mondiale attivo").setDescription(`Fonti: INGV (Italia) + USGS (mondo)\nControllo ogni ${I/1000}s.`).setTimestamp(new Date())]});console.log("Bot pronto");setInterval(()=>poll(ch),I);});
c.login(T);
